...
describe('F007: Baby texture transformations', () => {
  it('transforms veg components into soft/mashable guidance for baby', () => {
    ...
    expect(
      babyVariant.instructions.some(
        (i) =>
          i.includes('broccoli') &&
          i.includes('steam until very soft') &&
    ...
    expect(
      babyVariant.instructions.some(
        (i) => i.includes('sweet potato') && i.includes('steam until very soft'),
      ),
    ).toBe(true);
  ...
  it('excludes obviously unsafe non-baby-safe proteins', () => {
    ...
    expect(
      babyVariant.instructions.some(
        (i) => i.includes('Not suitable for baby') && i.includes('steak'),
      ),
    ).toBe(true);
  ...
  it('excludes hard-no foods in addition to baby-unsafe ingredients', () => {
    ...
    expect(
      babyVariant.instructions.some((i) => i.includes('Exclude') && i.includes('whole nuts')),
    ).toBe(true);
  ...
  it('keeps baby variant tied to the base meal when components can be adapted', () => {
    ...
    expect(babyVariant.instructions.some((i) => i.includes('broccoli'))).toBe(true);
    expect(
      babyVariant.instructions.some((i) => i.includes('tomato sauce') && i.includes('smooth')),
    ).toBe(true);
  });
  ...
  it('provides sauce-specific guidance for baby', () => {
    ...
    expect(
      babyVariant.instructions.some(
        (i) => i.includes('tomato sauce') && i.includes('no chunks') && i.includes('smooth'),
      ),
    ).toBe(true);
  });
  ...
  it('suggests safe food fallback when no baby safe food is in the meal', () => {
    ...
    expect(
      babyVariant.instructions.some(
        (i) => i.includes('No safe food in this meal') && i.includes('sweet potato'),
      ),
    ).toBe(true);
  });
