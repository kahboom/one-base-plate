...
describe("F060: Row selection", () => {
  ...
  it("select-all-on-page checkbox selects all items on current page", async () => {
    ...
    expect(screen.getByTestId("bulk-selected-count")).toHaveTextContent(`${DEFAULT_PAGE_SIZE} selected`);
  });
...
describe("F060: Selection persists across pagination", () => {
  it("selection persists when navigating pages", async () => {
    ...
    await user.click(screen.getByTestId("select-all-page-checkbox"));
    expect(screen.getByTestId("bulk-selected-count")).toHaveTextContent(`${DEFAULT_PAGE_SIZE * 2} selected`);
  });
...
describe("F060: Protected ingredient detection", () => {
  it("shows protected warning for ingredients referenced by meals", async () => {
    const ingredients = [
      makeIngredient({ name: "Protected" }),
      makeIngredient({ name: "Free" }),
    ];
...
describe("F060: findIngredientReferences utility", () => {
  ...
  it("detects references in weekly plan grocery lists", () => {
    const household: Household = {
      ...
      weeklyPlans: [{
        ...
        generatedGroceryList: [{ ingredientId: "ing-c", name: "C", category: "pantry", quantity: "1", owned: false }],
      }],
    };
  ...
  it("detects alternative ingredient references", () => {
    const household: Household = {
      ...
      baseMeals: [{
        id: "m1", name: "Meal",
        components: [{ ingredientId: "other", alternativeIngredientIds: ["ing-alt"], role: "protein", quantity: "1" }],
        defaultPrep: "", estimatedTimeMinutes: 30, difficulty: "easy", rescueEligible: false, wasteReuseHints: [],
      }],
      weeklyPlans: [],
    };
