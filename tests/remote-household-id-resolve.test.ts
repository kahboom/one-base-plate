...
function rh(cloudId: string, localId: string): RemoteHousehold {
  const data = { id: localId, name: "x", members: [], ingredients: [], baseMeals: [], weeklyPlans: [] } as Household;
  return {
    id: cloudId,
    data,
    owner_id: "o1",
...
describe("remote household id resolution", () => {
  ...
  it("maps local id to cloud pk via embedded household data", () => {
    ...
    expect(resolveRemoteHouseholdPkFromList("H001", remotes)).toBe("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee");
  });
