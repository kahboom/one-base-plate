...
describe("applyImportMappingEdit", () => {
  ...
  it("ignore → use inserts component in recipe order", () => {
    ...
    const { meal: next } = applyImportMappingEdit(meal, 0, { kind: "use", ingredientId: "ing-chicken" }, [
      ingChicken,
      ingRice,
    ]);
  ...
  it("use swaps ingredient on existing line", () => {
    ...
    const { meal: next } = applyImportMappingEdit(meal, 0, { kind: "use", ingredientId: "ing-rice" }, [
      ingChicken,
      ingRice,
    ]);
