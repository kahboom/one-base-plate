...
function makeRef(overrides: Partial<ComponentRecipeRef> & { id: string }): ComponentRecipeRef {
  return {
    componentId: 'comp-1',
...
describe('F058 — resolveFullCookingRef priority chain', () => {
  ...
  const rice = makeIngredient({
    ...
    name: 'rice',
    defaultRecipeRefs: [
      { recipeId: 'recipe-rice', label: 'Default rice method', role: 'component' },
    ],
  });
  ...
  const comp = makeComponent({
    ...
    recipeRefs: [makeRef({ id: 'cr-1', label: 'My chicken recipe', isDefault: true })],
  });
...
describe('F058 — findPrepAheadOpportunities', () => {
  ...
  it('returns empty when ingredients appear only once', () => {
    ...
    const days: DayPlan[] = [{ day: 'Monday', baseMealId: 'm1', variants: [] }];
...
describe('F058 — summarizeRecipeRef', () => {
  it('summarizes note refs', () => {
    expect(
      summarizeRecipeRef(
        makeRef({ id: '1', sourceType: 'note', notes: '200°C 25 min', label: 'Prep note' }),
      ),
    ).toBe('200°C 25 min');
  });
