...
describe('F010: Weekly plan generation engine', () => {
  it('generates the requested number of days', () => {
    const days = generateWeeklyPlan([mealPasta, mealRice], members, ingredients, 5);
    expect(days).toHaveLength(5);
  });

  it('generates 7-day plan by default', () => {
    const days = generateWeeklyPlan([mealPasta, mealRice], members, ingredients);
    expect(days).toHaveLength(7);
  });

  it('each day contains a base meal and per-person variants', () => {
    const days = generateWeeklyPlan([mealPasta, mealRice], members, ingredients, 3);
  ...
  it('avoids repeating the same meal on consecutive days when alternatives exist', () => {
    const days = generateWeeklyPlan([mealPasta, mealRice], members, ingredients, 4);
  ...
  it('reuses ingredients across the week intentionally', () => {
    const days = generateWeeklyPlan([mealPasta, mealRice, mealPastaRice], members, ingredients, 5);
    // Collect all ingredient IDs used
    const usedIds = new Set<string>();
    let totalComponents = 0;
    for (const day of days) {
      const meal = [mealPasta, mealRice, mealPastaRice].find((m) => m.id === day.baseMealId)!;
