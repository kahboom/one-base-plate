...
describe('F050 grouped Paprika resolution', () => {
  it('uses the same group key for hyphen vs space flour variants', () => {
    expect(normalizeIngredientGroupKey('All-purpose flour')).toBe(
      normalizeIngredientGroupKey('All purpose flour'),
    );
    expect(groupKeyForParsedName('all-purpose flour')).toBe(
      groupKeyForParsedName('all purpose flour'),
    );
  });
  ...
  it('migrateLegacy maps old unmatched+ignore to pending', () => {
    const parsed = parsePaprikaRecipes(
      [makeRecipe({ ingredients: '2 zzzuniquemysteryxyz' })],
      [],
      [],
    );
