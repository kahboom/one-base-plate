...
const toddler: HouseholdMember = {
  ...
  preparationRules: [
    { ingredient: "pasta", rule: "Must be small shapes like penne" },
  ],
  textureLevel: "soft",
  allergens: [],
  notes: "",
...
describe("F008: Toddler plate includes at least one safe food", () => {
  it("identifies safe food when meal contains a toddler safe food", () => {
    ...
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("Includes safe food") && i.includes("pasta"),
    )).toBe(true);
  });
  ...
  it("identifies multiple safe foods when meal contains several", () => {
    ...
    expect(toddlerVariant.safeFoodIncluded).toBe(true);
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("Includes safe food") && i.includes("pasta") && i.includes("cheese"),
    )).toBe(true);
  });
  ...
  it("suggests explicit fallback when no safe food is in the meal", () => {
    ...
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("No safe food in this meal") && i.includes("add on the side"),
    )).toBe(true);
    // Should suggest specific safe foods
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("pasta") || i.includes("bread") || i.includes("cheese"),
    )).toBe(true);
  });
  ...
  it("excludes hard-no foods and still checks safe food presence", () => {
    ...
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("Exclude") && i.includes("spicy sauce"),
    )).toBe(true);
  ...
  it("never silently omits toddler compatibility — always has explicit message", () => {
    ...
    expect(withoutSafe.instructions.some((i) => i.includes("No safe food in this meal"))).toBe(true);
  });
  ...
  it("handles toddler with no safe foods defined gracefully", () => {
    ...
    expect(toddlerVariant.safeFoodIncluded).toBe(false);
    expect(toddlerVariant.instructions.some((i) =>
      i.includes("No safe food matched") && i.includes("familiar side"),
    )).toBe(true);
  });
});
