...
describe("rankWeeklySuggestedMeals", () => {
  it("orders by overlap when no outcomes or plan", () => {
    const low = meal("low", "Low overlap", ["ing-c"], { estimatedTimeMinutes: 20, difficulty: "easy" });
    const high = meal("high", "High overlap", ["ing-a", "ing-b"], { estimatedTimeMinutes: 45, difficulty: "hard" });
  ...
  it("allows repeat when pinned", () => {
    ...
    const ranked = rankWeeklySuggestedMeals([onPlan, fresh], members, ingredients, [], ["on"], days);
  ...
  it("allows repeat when successCount >= 3", () => {
    ...
    const ranked = rankWeeklySuggestedMeals([favorite, other], members, ingredients, outcomes, [], days);
  ...
  it("boosts ingredient reuse vs current plan", () => {
    ...
    const shares = meal("shares", "Shares ing-a", ["ing-a", "ing-b"], { estimatedTimeMinutes: 30, difficulty: "medium" });
    const isolated = meal("iso", "Only ing-c", ["ing-c"], { estimatedTimeMinutes: 30, difficulty: "medium" });
    const planned = meal("planned", "Planned", ["ing-a"], { estimatedTimeMinutes: 30, difficulty: "medium" });
    ...
    const ranked = rankWeeklySuggestedMeals([isolated, shares, planned], oneAdult, ingredients, [], [], days);
    expect(ranked[0]!.meal.id).toBe("shares");
    expect(ranked[0]!.ingredientReuse).toBeGreaterThan(ranked.find((r) => r.meal.id === "iso")!.ingredientReuse);
  });
