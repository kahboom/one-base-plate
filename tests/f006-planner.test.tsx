...
const members: HouseholdMember[] = [
  ...
  },
  {
    ...
    hardNoFoods: ['mushrooms'],
    preparationRules: [{ ingredient: 'pasta', rule: 'Must be small shapes' }],
    textureLevel: 'soft',
    allergens: [],
    notes: '',
  },
...
describe('F006: Assembly variant generation engine', () => {
  ...
  it('excludes non-baby-safe ingredients for baby', () => {
    ...
    const sam = variants.find((v) => v.memberId === 'm-sam')!;
    expect(
      sam.instructions.some(
        (i) => i.includes('Not suitable for baby') && i.includes('chicken breast'),
      ),
    ).toBe(true);
  });
  ...
  it('generates variants for all household members', () => {
    ...
    expect(variants.map((v) => v.memberId)).toEqual(['m-alex', 'm-jordan', 'm-riley', 'm-sam']);
  });
});
