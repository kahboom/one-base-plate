...
- **Session restore fix:** `AuthProvider` now calls `setCurrentUserId(user.id)` when restoring an existing session on mount and on auth state changes. This means sync works after page refresh without requiring the user to visit the sign-in flow again. `main.tsx` wires `initOnlineListeners()` and `setLoadHouseholdsRef(loadHouseholds)` at startup.
