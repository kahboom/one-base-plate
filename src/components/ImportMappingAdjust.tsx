...
import {
  applyImportMappingEdit,
  type ImportMappingEditPayload,
} from "../import-mapping-edit";
...
export default function ImportMappingAdjust({
  ...
  function persist(index: number, payload: Parameters<typeof applyImportMappingEdit>[2]) {
    ...
    const merged = newIngredients.filter(
  ...
  return (
    <details className="mt-2 rounded-md border border-dashed border-border-light bg-bg/40 p-2">
      ...
      <div className="mt-2 space-y-2 border-t border-border-light pt-2" data-testid="import-mapping-adjust">
...
function ImportMappingRow({
  ...
  const useIngredientValue =
    mapping.action === "use" ? (mapping.finalMatchedIngredientId ?? mapping.ingredientId ?? "") : "";
  ...
  return (
    ...
    >
      <p className="min-w-0 flex-1 text-xs text-text-secondary line-clamp-2" title={mapping.originalLine}>
