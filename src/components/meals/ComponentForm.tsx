...
import type { BaseMeal, MealComponent, Ingredient, ComponentRecipeRef, Recipe } from '../../types';
import { Button, Input, Select, FieldLabel, Chip } from '../ui';
import { toSentenceCase, ingredientMatchesQuery } from '../../storage';
    ...
    >
            ...
            {alternativeIngredients.length > 0 ? (
              <div className="space-y-1.5">
                {alternativeIngredients.map((ing) => {
                  const altRef = (component.recipeRefs ?? []).find((r) =>
                    r.notes?.startsWith(`alt:${ing.id}`),
                  );
                  const altRefLabel = altRef
                    ? summarizeRecipeRef(altRef, {
                        linkedMealName: allMeals.find((m) => m.id === altRef.linkedBaseMealId)
                          ?.name,
                      })
                    : null;
                  return (
                    <div key={ing.id} className="flex flex-wrap items-center gap-1.5">
                      ...
                      {altRefLabel && (
                        <Chip
                          variant="neutral"
                          className="text-[10px]"
                          data-testid={`alt-recipe-chip-${ing.id}`}
                        >
                          Recipe: {altRefLabel}
                        </Chip>
                      )}
      ...
      {altRecipePickerIngId && component.id && (
        <ComponentRecipePicker
          ...
          onRemove={
            (component.recipeRefs ?? []).some((r) =>
              r.notes?.startsWith(`alt:${altRecipePickerIngId}`),
            )
              ? () => {
                  onChange({
                    ...component,
