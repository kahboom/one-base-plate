...
export default function Planner() {
  ...
  const [sessionOverrides, setSessionOverrides] = useState<Map<string, ComponentRecipeRef>>(
    new Map(),
  );
  const [pickerComponent, setPickerComponent] = useState<MealComponent | null>(null);
  const mealPlanRef = useRef<HTMLDivElement>(null);
  const trayCap = useSuggestedTrayCap();

  const dayParam = normalizeWeekdayParam(searchParams.get('day'));

  const regenerateVariants = useCallback((h: Household, mealId: string) => {
    if (!mealId) {
      setVariants([]);
      return;
    }
  ...
  function handleAddToWeekConfirm() {
    ...
    navigate(`/household/${householdId}/weekly`, {
      state: {
        ...
        assignComponentOverrides: overrides.length > 0 ? overrides : undefined,
  ...
  return (
    <>
      ...
      {dayParam && themeAnchor && (
        <p className="mb-3 text-sm text-text-secondary" data-testid="planner-theme-context">
          ...
          <Card className="mb-0">
            <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mb-3 text-sm text-text-secondary">
                  Prep: {selectedMeal.defaultPrep} | Time: {selectedMeal.estimatedTimeMinutes} min |
                  Difficulty: {selectedMeal.difficulty}
                </p>
                <div className="flex flex-wrap gap-2" data-testid="planner-primary-actions">
                  <Button
                    type="button"
                    variant="primary"
            ...
            <Section title="Shared base">
              <ul className="mb-4 space-y-1 pl-5 text-sm">
                {selectedMeal.components.map((c, i) => {
                  const ing = household.ingredients.find((ing) => ing.id === c.ingredientId);
                  return (
                    <li key={c.id ?? i}>
            ...
            >
              ...
              <p className="mb-3 text-xs text-text-muted">
                Defaults come from your base meal. “Tonight” overrides are only for this session
                until you add the meal to the week or save as defaults.
              </p>

              {(selectedMeal.recipeRefs ?? []).length > 0 && (
                <div
                  className="mb-4 rounded-md border border-border-light bg-surface-card p-3"
                  data-testid="whole-meal-recipe-refs"
                >
                  ...
                  <div className="mt-1 flex flex-wrap gap-2">
                    {(selectedMeal.recipeRefs ?? []).map((ref, i) => (
                      <Chip
                        key={ref.recipeId || i}
                        variant="info"
                        data-testid={`whole-meal-planner-ref-${i}`}
                      >
      ...
      >
        ...
        <p className="text-sm text-text-secondary">
          Pick a day. You can assign the meal by tapping the day card after you land on Weekly
          Planner.
        </p>
        ...
        <div className="flex gap-2">
          <Button
            variant="primary"
            onClick={handleAddToWeekConfirm}
