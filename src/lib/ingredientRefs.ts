  ...
  for (const meal of household.baseMeals) {
    ...
    if (meal.importMappings) {
      for (const m of meal.importMappings) {
        if (m.ingredientId) addRef(m.ingredientId, { type: "importMapping", entityName: meal.name });
        if (m.finalMatchedIngredientId) addRef(m.finalMatchedIngredientId, { type: "importMapping", entityName: meal.name });
      }
    }
  }
  ...
  for (const recipe of household.recipes ?? []) {
    ...
    if (recipe.importMappings) {
      for (const m of recipe.importMappings) {
        if (m.ingredientId) addRef(m.ingredientId, { type: "importMapping", entityName: recipe.name });
        if (m.finalMatchedIngredientId) addRef(m.finalMatchedIngredientId, { type: "importMapping", entityName: recipe.name });
      }
    }
  }
