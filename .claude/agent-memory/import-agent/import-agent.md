...
## Performance deep-dive (consolidated from import-performance-scaling, 2026-04-02)

_Originally audited 2026-03-31 against Entrees.paprikarecipes — 314 recipes, ~12.8 MB._
...
#### 3. `saveImportSession` — `src/pages/PaprikaImport.tsx` (useEffect)
...
#### 4. `reviewGroups.findIndex` per group row — `src/pages/PaprikaImport.tsx` ~line 1221
...
- `CATALOG_MIN = 0.86` is correctly conservative; many real ingredients won't fuzzy-match any catalog entry at this threshold

### Bulk review UX: scaling note
...
### Correctness guardrails (never weaken when optimizing)
...
| 2026-04-02 | Consolidated from orphan `.claude/agent-memory/import-performance-scaling/` into import-agent supporting notes.                                                                            |
